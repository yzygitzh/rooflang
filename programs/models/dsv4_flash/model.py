...
"""DeepSeek V4 Flash inference — Declaration Phase.
...
from rooflang.programs.models.dsv4_flash.config import (
...
def _build_layers(g, B, S, context_len, prev_out):
    ...
    for layer_id in range(N_LAYERS):
        ...
        bridge = Spawn(world=3 if is_prefill and ratio else 2)
        bridge.inputs = {"x": Tensor("bf16", (B, S, D))}
        bridge.outputs = {
            "y": Tensor("bf16", (B, S, D)),
            "y2": Tensor("bf16", (B, S, D)),
        }
        if is_prefill and ratio:
        ...
        if is_prefill:
            compressed_len = S // ratio if ratio else 0
            window_len = WINDOW
            ...
            compressed_len = context_len // ratio if ratio else 0
        ...
        if is_prefill:
            ...
            cache_src = kv_win_slice
            if ratio:
                # KV cache = concat(window_kv, compressed_kv)
                kv_concat = Concat()
                kv_concat.inputs = {
                    "a": Tensor("bf16", (B, WINDOW, KV_DIM)),
                    "b": Tensor("bf16", (B, compressed_len, KV_DIM)),
                }
                kv_concat.outputs = {
            ...
            g.add_data_edge(cache_src, kv_cache_quant, {"y": "x"})
        ...
        residual_port = "y3" if is_prefill and ratio else "y2"
...
def _build_decode_kv_cache_read(g, B, context_len, layers):
    ...
    for layer_id, layer in enumerate(layers):
        ratio = COMPRESS_RATIOS[layer_id]
        cache_len = min(WINDOW, context_len)
        if ratio:
            cache_len += context_len // ratio
