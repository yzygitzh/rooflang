...
from rooflang.language.optimization.split import (
    batch_split, context_split_decode, context_split_prefill,
...
from rooflang.programs.models.glm_5_2_fp8.config import (
    INDEX_TOPK, N_EXPERTS, TOPK,
)


_PREFILL_PARALLEL_FIELDS = (
    "bridge", "attn_norm", "attn_fan", "wq_a", "q_norm", "q_fan",
    "wq_b", "wkv", "kv_norm", "kv_cache_quant", "kv_persist_fan",
    "sa", "wo", "attn_add", "index_wq", "index_wk", "index_norm",
    "index_weights", "index_cache_quant", "index_cache_fan",
    "ffn_bridge", "ffn_norm", "ffn_fan", "dense_up", "dense_down",
...
# Decode computes one Q/current-KV path per DP shard and broadcasts Q/indexer
# inputs at the attention boundary. The post-attention dense and MoE work is
# split across DP×CP, matching dsv4_pro's attention/FFN placement policy.
_DECODE_DP_ONLY_FIELDS = (
    "bridge", "attn_norm", "attn_fan", "wq_a", "q_norm", "q_fan",
    "wq_b", "wkv", "kv_norm", "kv_sink", "index_wq", "index_wk",
    "index_norm", "index_weights", "index_sink",
)

_DECODE_SHARDED_FIELDS = (
    "wo", "attn_add", "ffn_bridge", "ffn_norm", "ffn_fan",
    "dense_up", "dense_down", "gate", "dispatch", "combine", "sw_up",
    "sw_down", "moe_add", "ffn_add",
)
...
def _validate_args(
    ...
    if seq_prefill % cp != 0:
        raise ValueError(
            ...
            f"cp={cp}")
    if seq_prefill >= INDEX_TOPK and INDEX_TOPK % cp != 0:
        raise ValueError(
            f"INDEX_TOPK={INDEX_TOPK} must be divisible by cp={cp}")
...
def _set_expert_weight_read_fraction(
    ...
    Dense layers retain a zero fraction because they own no routed experts.
    """
    ...
    for layer in layers:
        layer._expert_weight_read_fraction = (
            read_fraction if layer.experts else Fraction(0))
...
def optimize_model_cluster_prefill(
    ...
    # once. The input Spawn has multiple consumers (attention and residual
    # paths); leaving its aliases in source HBM repeats the same remote read.
...
def optimize_model_cluster_decode(
    ...
    for layer in layers:
        field_names = [
            name for name in (
                ...
                *_DECODE_REPLICATED_FIELDS,
                *_DECODE_SHARDED_FIELDS,
                "sa", "kv_cache_fan", "index_cache_fan",
            )
            if getattr(layer, name) is not None
        ]
        dp_fields = {}
        ...
        layer_copies = {
            "dp": {
                name: dp_fields[name]
                for name in _DECODE_DP_ONLY_FIELDS
                if name in dp_fields
            },
            "cp_dp": {},
        }
    ...
    for layer, dp_fields, layer_copies in zip(
        ...
        cp_fields = {}
        for name in ("kv_cache_fan", "index_cache_fan", "sa"):
